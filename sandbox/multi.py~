...
def main(args):
    ...
    dtrain = bolt.io.MemoryDataset(dim,itrain,ltrain)
    dtest = bolt.io.MemoryDataset(dim,itest,ltest)

    cats = news.categories()
    cats = dict(((i,c) for i,c in enumerate(news.categories())))
    k = len(cats)

    model = MLinearModel(dim,k)
    ova = OVA(loss = bolt.ModifiedHuber(), reg = 0.0001)
    ova.train(model,dtrain, epochs = 10)
    ...
    #f1 = nltk.metrics.f_measure(ref,pred)
